$(document).ready(() => {
  ...
  $('#toggleLangNav').on('click', function(e) {
    e.preventDefault();
    const lang = getUILang() === 'zh-CN' ? 'en-US' : 'zh-CN';
    setUILang(lang);
    applyI18n();
    $('#toggleLangNav').text(lang === 'zh-CN' ? 'EN' : '中文');
  });
  $('#toggleLangNav').text(getUILang() === 'zh-CN' ? 'EN' : '中文');
...
function streamHtmlTemplate(uuid, name) {
  return '<div class="item" id="' + uuid + '">' +
    ...
    '<a class="dropdown-item" onclick="showShareModal(\'' + uuid + '\')" href="#">' + t('share_video') + '</a>' +
...
function showShareModal(uuid) {
  Swal.fire({
    title: t('share_modal_title'),
    html: '<div class="text-left">' +
      '<div class="form-group">' +
      '<label>' + t('share_minutes') + '</label>' +
      '<input id="share-expire" type="number" min="1" max="1440" class="form-control" value="60">' +
      '</div>' +
      '<div class="form-group">' +
      '<label>' + t('share_max_conn') + '</label>' +
      '<input id="share-max-conn" type="number" min="1" max="5" class="form-control" value="1">' +
      '</div>' +
      '<div class="form-group">' +
      '<label>Channel</label>' +
      '<input id="share-channel" type="number" min="0" class="form-control" value="0">' +
      '</div>' +
      '</div>',
    showCancelButton: true,
    confirmButtonText: t('share_create'),
    cancelButtonText: t('cancel'),
    preConfirm: () => {
      const channelVal = parseInt($('#share-channel').val() || '0', 10);
      return $.ajax({
        url: '/share/create',
        type: 'POST',
        ...
        data: JSON.stringify({
          uuid: uuid,
          channel: isNaN(channelVal) ? '0' : String(channelVal),
          expire_minutes: parseInt($('#share-expire').val() || '60', 10),
          max_connections: parseInt($('#share-max-conn').val() || '1', 10)
        })
      }).then((resp) => {
        if (!resp || resp.status !== 1) {
          throw new Error(t('share_failed'));
        }
        return resp.payload;
      }).catch((xhr) => {
        let message = t('share_failed');
        if (xhr && xhr.responseJSON && xhr.responseJSON.payload) {
          message = xhr.responseJSON.payload;
        }
    ...
    Swal.fire({
      title: t('share_modal_title'),
      html: '<div class="text-left">' +
        '<p><strong>' + t('share_password') + ':</strong> ' + payload.password + '</p>' +
        '<p><strong>' + t('share_link') + ':</strong><br><a target="_blank" href="' + payload.share_url + '">' + payload.share_url + '</a></p>' +
        '<p><strong>' + t('share_qr') + ':</strong></p>' +
        '<div class="text-center"><img alt="qr" style="max-width:220px;" src="' + payload.qr_url + '"/></div>' +
        '</div>',
...
function initRecordControls() {
  if ($('#recordBtn').length === 0 || $('#uuid').length === 0) {
    return;
  }
  let recording = false;
  const uuid = $('#uuid').val();
  const channel = $('#channel').val() || '0';
  const btn = $('#recordBtn');
  const btnLabel = $('#recordBtnLabel');
  const info = $('#recordInfo');

  function setRecordState(active, text) {
    recording = active;
    if (active) {
      btn.addClass('recording');
      btnLabel.text(t('stop'));
    } else {
  ...
  function refreshStatus() {
    $.get('/stream/' + uuid + '/channel/' + channel + '/record/status', function(resp) {
      if (!resp || resp.status !== 1) {
        return;
      }
      const payload = resp.payload || {};
      setRecordState(!!payload.active, payload.file_path || '');
    });
  }
  ...
  btn.on('click', function() {
    ...
    $.ajax({
      url: '/stream/' + uuid + '/channel/' + channel + '/record/' + action,
      type: 'POST',
      success: function(resp) {
        if (!resp || resp.status !== 1) {
          Swal.fire({ icon: 'error', title: t('recording_error'), confirmButtonText: t('ok') });
          return;
        }
        if (action === 'start') {
          setRecordState(true, resp.payload.file_path || '');
          Swal.fire({ icon: 'success', title: t('recording_started'), timer: 1000, showConfirmButton: false });
        } else {
          setRecordState(false, resp.payload.file_path || '');
          Swal.fire({ icon: 'success', title: t('recording_stopped'), timer: 1000, showConfirmButton: false });
        }
      },
      error: function(xhr) {
        let text = t('recording_error');
        if (xhr && xhr.responseJSON && xhr.responseJSON.payload) {
          text = xhr.responseJSON.payload;
        }
        Swal.fire({ icon: 'error', title: text, confirmButtonText: t('ok') });
      }
    });
  });

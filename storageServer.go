...
// ServerUILanguageDefault returns default UI language.
func (obj *StorageST) ServerUILanguageDefault() string {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	if obj.Server.UILanguageDefault == "" {
		return "zh-CN"
	}
	return obj.Server.UILanguageDefault
}

// ServerRecording returns recording configuration.
func (obj *StorageST) ServerRecording() RecordingST {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	return obj.Server.Recording
}

// ServerShare returns share configuration.
func (obj *StorageST) ServerShare() ShareST {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	return obj.Server.Share
}

// ServerDetection returns detection configuration.
func (obj *StorageST) ServerDetection() DetectionServerST {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	return obj.Server.Detection
}

// ServerAdminPasswordHash returns admin password hash.
func (obj *StorageST) ServerAdminPasswordHash() string {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	return obj.Server.AdminPasswordHash
}

// ServerSessionTTLMinutes returns admin session TTL in minutes.
func (obj *StorageST) ServerSessionTTLMinutes() int {
	obj.mutex.RLock()
	defer obj.mutex.RUnlock()
	return obj.Server.SessionTTLMinutes
}
